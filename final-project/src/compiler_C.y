...
%{	
	...
	void PrintingCcode(struct Gdeclsec * Gl, struct Fdefsec * Fn, struct Main_B * Mn);
	...
	void PrintingMainBlock_C(struct Main_B * Mn);
	void print_expr_C(struct exper * ex);
	void print_statement_C(struct stmtlist * st);
	void print_assign_C(struct argstmt * curr);
	void print_funCall_C(struct funccall * fun_cs);
	void printc();
	
	void PrintingFdef_sec_C(struct Fdefsec * Fn);
	void PrintingLdecl_sec_C(struct Ldecllist * list);
	void Printingret_stmt_C(struct ret * rt);
	
	void PrintingGdecl_sec_C(struct Gdeclsec * Gl);
	void PrintingGdecl_list_C(struct Gdeclsec_list * list);
	void Printingarg_list_C(struct arglist * curr);
%}
...
%%
	...
	Prog	:	Gdecl_sec  Fdef_sec  MainBlock{ PrintingCcode($1,$2,$3); }
		;
	...
	stmt_list:				{ $$ = (struct stmtlist *)malloc(sizeof(struct stmtlist)); }
...
void PrintingGdecl_sec_C(struct Gdeclsec * Gl){
	while(Gl->lst != NULL){
		PrintingGdecl_list_C((Gl->lst)->val);
		(Gl->lst) = (Gl->lst)->next;
		printf(";\n");
	}
}
...
void PrintingGdecl_list_C(struct Gdeclsec_list * list){
	if(list != NULL){
		...
		else if((list->Datatype)->types == 2) printf("int ");
		
		while(list->lst != NULL){
			
			if((list->lst)->globals != NULL){
				struct G_id * curr = (list->lst)->globals;
				if(curr->arr) printf("%s[%d]", curr->name, curr->ind);
				else printf("%s",curr->name);
			}
			else if((list->lst)->funs != NULL){
				printf("%s(",((list->lst)->funs)->name);
				Printingarg_list_C(((list->lst)->funs)->lst);
	...
	if(curr != NULL){
		...
		while((curr->lst) != NULL){
			...
			while(crr->vari_lst != NULL){
				...
				else if((crr->Datatype)->types == 2) printf("int ");
				
				printf("%s", (crr->vari_lst)->vari);
...
void PrintingFdef_sec_C(struct Fdefsec * Fn){
	while(Fn != NULL){
		if(((Fn->val)->Datatype)->types == 1) printf("int ");
		else if(((Fn->val)->Datatype)->types == 2) printf("int ");
		
		printf("%s (",(Fn->val)->name);
		Printingarg_list_C(((Fn->val)->lst)->lst);
		printf("{\n");
		
		if((Fn->val)->locals != NULL) PrintingLdecl_sec_C(((Fn->val)->locals)->lst);
		
		print_statement_C((Fn->val)->st);
		Printingret_stmt_C((Fn->val)->returns);
		printf("}\n");
...
void PrintingLdecl_sec_C(struct Ldecllist * list){
	while(list != NULL){
		...
		else if(((list->val)->Datatype)->types == 2) printf("int ");
		
		while((list->val)->lst != NULL){
			
			printf("%s",((list->val)->lst)->var);
...
void PrintingMainBlock_C(struct Main_B * Mn){
	if((Mn->Datatype)->types == 1) printf("int ");
	else if((Mn->Datatype)->types == 2) printf("int ");
	printf("main(){\n ");
	if(Mn->locals != NULL) PrintingLdecl_sec_C((Mn->locals)->lst);
	print_statement_C(Mn->st);
	Printingret_stmt_C(Mn->returns);
	printf("}\n");
}
...
void print_statement_C(struct stmtlist * st)
{
	while(st->next != NULL){
		if((st->val)->ind == 1){
			...
			if(curr->ind == 1){
				printf("if( ");
				print_expr_C(curr->check);
				printf(")\n{");
				print_statement_C(curr->s1);
				printf("}\n");
			}
			else if(curr->ind == 2){
				printf("if( ");
				...
				printf("for (");
				print_assign_C(curr->arg1);
				print_expr_C(curr->check);
				printf(";");
				for_loop = 1;
				print_assign_C(curr->arg2);
				for_loop = 0;
				printf("){\n");
				print_statement_C(curr->s1);
				printf("}\n");
			...
			if((curr->vari)->arr){
				printf("%s[",(curr->vari)->vari);
				print_expr_C((curr->vari)->ind);
				printf("] ");
			}
			else printf("%s ",(curr->vari)->vari);
			printf(");\n");
			
		}
		else if((st->val)->ind == 5){
			print_funCall_C(((st->val)->funs)->fun_c);
			printf(";");
...
void print_assign_C(struct argstmt * curr){
	...
	if(for_loop == 0) printf(";");
}
void print_funCall_C(struct funccall * fun_cs){
	printf("%s(",fun_cs->vari);
	if((fun_cs->p_list) != NULL){
		while(((fun_cs->p_list)->p1_list)->next != NULL){
			print_expr_C((((fun_cs->p_list)->p1_list)->p)->val);
			printf(",");
...
void print_expr_C(struct exper * ex)
{
	if(ex == NULL) return;
	if(ex->fun_c != NULL){
		print_funCall_C(ex->fun_c);
		return;
	}
	if(ex->vari != NULL) {
		if((ex->vari)->arr){
			printf("%s[",(ex->vari)->vari);
			print_expr_C((ex->vari)->ind);
			printf("] ");
		}
		else printf("%s ",(ex->vari)->vari);
		return;
	}
	...
	if(ex->opr == 'a'){ 
		...
		printf("( ");
		print_expr_C(ex->left);
		printf(") ");
	}
	else if(ex->opr == '5'){ 
		printf("LOGICAL_NOT ");
		print_expr_C(ex->right);
	}
	else{
		print_expr_C(ex->left);
		switch(ex->opr) {
...
void PrintingCcode(struct Gdeclsec * Gl, struct Fdefsec * Fn, struct Main_B * Mn){
	...
	PrintingGdecl_sec_C(Gl);
	printf("\n");
	PrintingFdef_sec_C(Fn);
	PrintingMainBlock_C(Mn);
}
